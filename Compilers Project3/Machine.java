	...
	String check(
		...
		ArrayList resWord, Lex l) {
			...
			if (c){//if there were alpha characters
				...
				if(tok.length() > 10){
				...
				}else if(tok.toUpperCase().equals("PROGRAM")){
					...
					if(tok.toUpperCase().equals("AND")){
						...
						Token t = new Token(tok, Token.ID, 0, lineNum);
						tokenFile.printToken(t);
						l.addToken(t);
					}
					return rval;
...
class NumMachine extends Machine {
	...
	String check(
		...
		if (c) { //if there were digits check the next character?
			...
			if ((str.charAt(f)) == '.' && !((str.charAt(f+1)) == '.')) { //if there is a decimal point
